...
def specaugment(spec, W=60, F=27, T=70, num_freq_masks=2, num_time_masks=2, p=0.2, replace_with_zero=False):
	...
	#	spec = freq_mask(spec, sF=F, num_masks=num_freq_masks, pad_value=pad_value)
	if random.random() <= 0.5:
		spec = time_mask(freq_mask(spec, F=F, num_masks=num_freq_masks, pad_value=pad_value), T=T, num_masks=num_time_masks, p=p, pad_value=pad_value)
